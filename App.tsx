...
// Separate type import for better module resolution
...
const App: React.FC = () => {
  ...
  // Logic to distinguish between preview and production environments
  useEffect(() => {
    const isNetlify = window.location.hostname.includes('netlify.app');
    
    // In local preview mode (outside netlify), always grant access
    if (!isNetlify) {
      setHasAccess(true);
      return;
    }

    // On Netlify, enforce the access key
    ...
    if (accessKey === SECRET_PW || isAlreadyAuth) {
      ...
      // Clean up the URL if the key was provided
      if (accessKey) {
  ...
  // Render Access Denied UI if on Netlify without valid key
  if (!hasAccess) {
    return (
